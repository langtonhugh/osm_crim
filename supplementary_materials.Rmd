...
We can now easily visualize this data (see Figure 2) using the `ggplot2` package, which is compatible with `sf` objects.
...
We can now plot the Open Street Map station points, which are already spatial, over these station locations pulled from the TfL API, to see how the two datasets compare (see Figure 3).
...
We can then visualize these counts and compare the two sources of data by coloring in our buffers according to the crime count (see Figure 4). Note that we arrange the plots using syntax available using the `patchwork` package, and set the scales to be comparable using the `lim` argument within `scale_color_viridis_c()`.
