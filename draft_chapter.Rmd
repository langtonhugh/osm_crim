...
Open data practices, and the extent to which it is a reality, vary both within and between countries. For example, the United States has a history of making public sector data sets openly available. The United Kingdom can be more strict, releasing data under a licence ('Open Government Licence'), with some data requiring a fee [@kitchin2014data]. The _Global Open Data Index_ [@globopendata] is one tool for tracking the annual global benchmark for publication of open government data by country. It provides a rank which indicates how well governments across the world follow open data practices across varoius domains. Many of these have direct relevance to crime and place research, such as administrative boundaries and geographic locations (e.g. coordinates).
...
The Public Sector refers to organizations owned and operated by local or federal government with the core aim to provide services to the public. Much of the open data movement has focused on opening up information generated by these local and national state agencies [@kitchin2014data]. These can include national statistics, or administrative data. Another set of data included in this group is data collected by publicly funded research, for example victimisation surveys, housing surveys, and others. 


The _Global Open Data Index_ introduced earlier focuses on identifying the gaps in governmental organizations, encouraging them think about how public sector information can become more usable, and ultimately, more impactful [@globopendata].
...
data describing urban structure, such as street networks to assess whether the configuration of streets dictates things like violent crime victimisation [@streetsyntax2017], or victimisation surveys mentioned earlier to quantify citizens' perceived safety and security. 
...
Contrary to public sector, private sector refers to the rest of the economy whihc are not under direct state control. Opening up data generated by the private sector represents a significant challenge, largely due to the proprietary value to its creators [@kitchin2014data]. The aims and objectives of private companies, who have a duty to shareholders and operate in a competitive market environment, differ considerably from local and central government. 
...
With this in mind, we now move on to a practical exercise, where we will utilize mutliple sources of open data, and explore police recorded crime on and around public transport in London, England.
...
- Accessing open data using 3 different methods: 
    + direct downloads 
...
- How to engage with the critical 'Where', 'Why', 'Who', 'What', 'When' questions to better understand open data.

## Our aim

In this exercise we will explore crime in and around London Underground stations. We know that there are important environmental features when it comes to public transport areas being more or less criminogenic environments. Studies in Sweden, for instance, have shown the importance of environmental and neighborhood characteristics in determining crime concentrations at underground stations in Stockholm, along with the positioning of stations on the line [@ceccato2013security]. Similar work has been done exploring bus stops in Los Angeles [@loukaitou1999hot] and the impact of intensive policing along bus corridors in Merseyside, England [@newton2004crime], amongst others. 


Now we will consider the case of London. Specifically, we will examine to what extent does crime cluster in and around London Underground stations?

We will use various sources of open data to answer this question, and this will allow us to explore the strengths and limitations across public sector and crowdsourced open data sources. 
...
In the United Kingdom, police recorded crime data in England and Wales can be accessed this way using an online web portal (https://data.police.uk/) under Open Government Licence. 


Visit the website above, and you will see a welcome message, and 6 tabs across the top of it, which should read "Home", "Data", "API", "Changelog", "Contact", and "About". 

Before we download any data, we can learn more about it by clicking on the "About" tab. This brings up a lot of information that it is important to carefully read through to be able to answer the where, why, who, what, when questions posed earlier. Take a moment to read through, and try to take notes on what you think might be relevant for your analysis. For example, if we want to map crimes, we might want to explore if there is any type of "anonymisation" that might take place before the data are released (to protect the privcy of the victims for example). If you read the about page here, you might find the following note: 
...
This indicates that although we get a latitude and longitude coordinate with each crime event, it may only be approximate, and this may have implications for our findings later!


To then actually download some data, move on to clicking on the "Data" tab. This should open for you a page titled "Data downloads" where you can see another 5 tabs: "Custom download", "Archive", "Boundaries", "Open data", and "Statistical data".  Staying on this "Custom download" page you can make some selections on what sort of data you wanted to download. 


We can select the time period and police force of interest, and the type of information required (e.g. crimes, stop and search, outcomes). We are also informed that the data are downloaded in comma-separated values format (.csv file extension), which is in line with machine-readable, easy to manipulate data format requirements. 
...
Save this file locally (in your working directory) to a subfolder named "data", and then you can load it in with the `read_csv()` command from the `readr` package (remember to load it with `library(readr)`). 
...
Note: If you didn't manage to follow along with the download instructions, you can also access this dataset from our github page: 
...
You should have now a dataframe with `r nrow(btp_df)` crimes in them ready for you to explore!
...
Here we will make a direct request to the API created by London's local government organization responsible for transport: Transport for London (TfL). TfL oversee the London Underground, and provide access to their open data through their unified API. Much like how we found the "About" page when downloading police data directly, to answer our 'Where', 'Why', 'Who', 'What', 'When' questions, we must find a similar document here. Details about the TfL API are provided through their open data page (https://tfl.gov.uk/info-for/open-data-users/unified-api), which explains what data is available, and how the API is designed. 
...
To draw conclusions about crimes at London Underground stations, we will need some data specifically about these underground stations. 


In the documentation there 'API area' (which offer some guidance on the types of data available). Scroll down and find the example calle for stops _Stops_. In the description you will  see that this call returns information on stops for busses or London Underground lines, and there are two examples (bus route 24 and the Bakerloo line). Take their demonstration url you see for the bakerloo line (https://api.tfl.gov.uk/line/bakerloo/stoppoints) and paste it into your web browser. When you visit this page you should see something like Figure 1:
...
Indeed, all you need to do is replace "bakerloo" with "northern", and you got the stops for this route instead. Now let's see how we would make this call in R. 


In R, we can use the `fromJSON()` function from the `rjson` package to parse all this information into a data frame (with rows and columns). It will then become more familiar and usable. All we need to do is input the URL from the TfL API into the function, with a bit of help from `readLines()`, another function in base R which is used to read text from URLs.
...
If you run this, you will see that each element in `api_call` is a "list". 


This demonstrates an important challenge faced by researchers when using open data, because dealing with data in this format can be messy and complicated, not always a neatly formatted dataframe like the csv from the police.uk open data portal. But we will learn how to manage this data into a more familiar format later. 


### Request to an API using a wrapper

Finally, the last way we will look at how open data is often shared is by using API wrappers. Often, developers who work with APIs will share their code, and release them in the form of a package or module, so that other people can use it too. This is called a _wrapper_ because it uses code that 'wraps' around the API to make it a neater, more usable package. Wrappers remove (or at least lower) many of the obstacles to accessing open data noted earlier. The wrapper can take many forms, such as a Python module, or an R package. It could even be a web interface that provides a graphical user interface (GUI) for accessing the API in question. 
...
If we load and use the package `osmdata` we can use its funcitons to query the Open Street Map API, rather than the API query being made directly (like we did for TfL, using the URL). Once again, we want to have some documentation to be able to refer to about our data and the ways to query it. When it comes to OSM, you can read all about it on their community page: [https://www.openstreetmap.org/about](https://www.openstreetmap.org/about). Regarding the wrapper, as it is an R package, it will come with a handy vignette, which in this case we can find at [https://cran.r-project.org/web/packages/osmdata/vignettes/osmdata.html](https://cran.r-project.org/web/packages/osmdata/vignettes/osmdata.html). 

Unlike the TfL API, `osmdata` is an international database, and has lots of data that we might not necessarily need. The first thing we need to specify therefore, is where we want to get data from. To do this, we use the `getbb()` function from the `osmdata` package, which stands for "get bounding box". You can think of the bounding box as a box drawn around an area that we are interested in (in this case, London, UK) which tells the OSM API that we want everything *inside* the box, but nothing *outside* the box. 

So how can we name a bounding box specification to define the study region? This can be obtained manually, which requires some existing knowledge about an area using the latitude and longitude coordinates, or you can use a search term. Here, we want to select Greater London, in the UK so we can use the search term "greater london united kingdom". Besides specifying where we want data from, we can also tell the `getbb()` function what format we want the data in. Let's say in this case we want a spatial object, specifically an `sf` polygon object which we name `bb_sf`:
...
We now have our study region defined as the administrative boundaries of Greater London, which we can visualize quickly to check it has worked as expected. To visualise we can use the `sf` and `ggplot2` packages. 
...
If you are familiar with the shape of London then this image should be familiar as well! Pleased with our bounding box, we can now move on to scrape data from the Open Street Map API using the `opq()` function. The function name is short for 'Overpass query', which is how users can query the Open Street Map API using search criteria. 
...
We do this using the `add_osm_feature()` function, specifying our key as 'public transport' and our value as 'station'. 

We also want to specify what sort of object (what class) to get our data into, and as we're still working with spatial data we stick to the `sf` format, for which the function is `osmdata_sf()`. 

And finally, we trim everything down into our bounding box parameters with the `trim_osmdata()` function. We add all these together using a pipe (`%>%`) and make the call like so: 
...
This confirms details like the bounding box coordinates, but also provides information on the features collected from the query. As one might expect, most information relating to public transport station locations has been recorded using points (i.e. two-dimensional vertices, coordinates) of which we have over seven thousand at the time of writing. We also have around one hundred polygons. 

For now, let's extract the point information.

```{r stationpoints}
osm_stat_sf <- osm_stat_sf$osm_points 
```

We now have a simple features object with all the public transport stations in London, which the volunteers of OSM have mapped, and also 130 variables of auxiliary data they collected, such as the "fare_zone" the station is in, what "amenity" it may have, whether it has "toilets", and many many more. 

Of course, this is up to the volunteers whether they collect all these data, and in many cases they just might not. For example, if we look at fare zone, we might see the following: 
...
You can see for most of these stations (`r osm_stat_sf %>% group_by(fare_zone) %>% count() %>% st_drop_geometry() %>% filter(is.na(fare_zone)) %>% pull(n)`) we have missing data (denoted by NA). Nevertheless, when the details are recorded, they provide rich insight and local knowledge we may otherwise not attain. 


One last thing to do is to select the stations which are relevant to us (ie: they fall along the Northern Line). The variable `line` can help us with this. First let's look at the values in this variable by using the `unique()` function: 
...
You might see that Northern appears a few times, once as "Northern" but also once as "Northern City". As we know there is no line called "Northern City" and we can see many other combinations in these values (eg Circle;District, indicating the station serves both Circle and District lines) we can assume that this is a station that serves both Northern line and one other. 


To select all Northern line stations we can use the `grepl()` function to collect everything with "Northern" in the name: 
...
And now we have all our open data sets together in one place!


## Cleaning, wrangling, and visualising our data

Just because we have our data doesn't necesarily mean it's in the shape we want yet. The first thing to do once we have acquired our open data is investigate it, using data cleaning, wrangling, and visualisation. 

We've already loaded the `dplyr` and `sf` and `ggplot2` packages, but here we will also make use of `tidyr` and `patchwork`, so load these (remember, if you do not have these packages installed, use the `install.packages()` function prior to loading each one with `library()`.)
...
We usually want to work with data that is in the dataframe format, that is all your observations are rows and all your variables are columns. While this is the case for our police data and our OSM data, recall that the TfL data came in the nested JSON format.
...
We can then plot these OSM station points over the station locations pulled from the TfL API, to see how the two data sets compare. We do not plot the study region boundary in order to get a more detailed, local view.
