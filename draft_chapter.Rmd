...
\dag School of Law, University of Leeds, Leeds, United Kingdom. ORCID: 0000-0002-1322-1553.

\ddag School of Social Sciences, University of Manchester, Manchester, United Kingdom. ORCID: 0000-0001-8689-1526.
...
Open Street Map has two main types of wrappers available for its API, a web-based GUI called Overpass Turbo (https://overpass-turbo.eu/), and an R package called `osmdata`. 
...
We can now plot the Open Street Map station points, which are already spatial, over these station locations pulled from the TfL API, to see how the two datasets compare.
...
We can then visualize these counts and compare the two sources of data by coloring in our buffers according to the crime count. Note that we arrange the plots using syntax available using the `patchwork` package, and set the scales to be comparable using the `lim` argument within `scale_color_viridis_c()`.

```{r crimemapcomparesave, echo = F}
...
```{r mapfig, out.width = "\\textwidth", echo = F}
...
# Open Street Map (or TfL) buffer. Note that we transform to WGS 84 first in preperation
# for the Open Street Map API.
bb_sf <- stratford_osm_sf %>%
  st_transform(crs = 4326) %>% 
  st_bbox() 

# Retrieve the coordinates of the bounding box, and create a numeric list for use with
# osmdata package.
...
```{r strat, echo = F}
knitr::include_graphics("img/stratford.png")
```

This quick check has shed some light on what is happening. The buffer generated from the TfL station location (in blue) covers a different part of the station to the Open Street Map buffer (in pink). The latter covers most of the main station building, while the former centers around the platforms to the north. Because the British Transport Police crime locations have been geocoded to a location in the main station building (in red), the TfL buffer has missed them. If we had used a larger buffer, it is likely that the TfL location would have captured the crime cluster, but then we would increase the risk of picking up crimes outside of the station. There is no obvious 'right or wrong' answer to determine which open data source is most appropriate. Here, we might consider investigating other Jubilee stations before deciding which data to use, and ensure that findings are reported transparently with reference to these discussions.

In many cases, engaging with the creators of the data would be an important step in answering the critical questions ('Where', 'Why', 'Who', 'What' and 'When') posed at the beginning of this chapter. For TfL, this might mean investigating the criteria used by their analysts to define the coordinates for stations. For Open Street Map, its online transparency means that we could find out which user(s) recorded the Jubilee station locations, and directly contact this community of volunteers to find out how station locations were recorded and why. 
